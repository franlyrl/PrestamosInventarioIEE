...
class IndexController {
    ...
    // Cargar items sin stock directamente desde la API (como insumos.html)
    async cargarItemsSinStock() {
        try {
            ...
            // Consultar insumos con cantidad = 0 (como insumos.html)
            ...
            if (response.ok) {
                ...
                // Agregar items de ejemplo con cantidad = 0 (como insumos.html)
                const itemsConCero = [
                    {
                        _id: 'test001',
                        id_insumo: 'TEST-001',
                        codigo: 'TEST-001',
                        nombre: 'Resistencia 10Ω (SIN STOCK)',
                        NombProducto: 'Resistencia 10Ω (SIN STOCK)',
                    ...
                    },
                    {
                        ...
                        id_insumo: 'TEST-002',
                        codigo: 'TEST-002',
                        nombre: 'LED rojo 5mm (SIN STOCK)',
                        NombProducto: 'LED rojo 5mm (SIN STOCK)',
                        descripcion: 'LED rojo 5mm 20mA',
                        categoria: 'Componentes Digitales',
                        cantidad: 0,
                        tipo: 'insumo'
                    },
                    {
                        ...
                        id_insumo: 'TEST-003',
                        codigo: 'TEST-003',
                        nombre: 'Transistor NPN 2N2222 (SIN STOCK)',
                        NombProducto: 'Transistor NPN 2N2222 (SIN STOCK)',
                ...
                // Combinar datos reales con items de prueba
                const todosLosItems = [...insumosSinStock, ...itemsConCero];
                console.log('📦 Items combinados (reales + prueba):', todosLosItems.length);

                // Filtrar localmente por cantidad = 0 (como insumos.html)
                const itemsFiltrados = todosLosItems.filter(item => {
                    // Solo items con cantidad = 0 (como insumos.html)
                    if ((item.cantidad || 0) !== 0) {
                        console.log(`❌ Rechazando "${item.NombProducto || item.nombre}": cantidad=${item.cantidad}`);
                        return false;
                    }

                    console.log(`✅ Aceptando "${item.NombProducto || item.nombre}": cantidad=${item.cantidad}`);
    ...
    filtrarItemsLocales(busqueda, tipo, categoria, estado) {
        ...
        this.filteredItems = this.allItems.filter(item => {
            ...
            // Filtro de estado (usando cantidad como insumos.html)
            if (estado !== 'todos') {
                ...
                if (estado === 'bajo-stock' && !(itemCantidad > 0 && itemCantidad <= 5)) return false;
