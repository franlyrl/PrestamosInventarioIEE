...
class IndexController {
    ...
    crearItemCard(item) {
        ...
        const btnCarrito = cantidad > 0
            ? ('<button class="flex-1 bg-utn-blue text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-utn-dark transition" onclick="event.stopPropagation(); addToCart(this.dataset.nombre, this.dataset.tipo, JSON.parse(this.dataset.item), this)" data-nombre="' + nombre.replace(/"/g,'') + '" data-tipo="' + tipo + '" data-item="' + itemSafe + '">Añadir</button>')
            : ('<button class="flex-1 border border-utn-blue text-utn-blue px-3 py-2 rounded-lg text-xs font-bold hover:bg-blue-50 transition" onclick="event.stopPropagation(); window.estDash?.entrarListaEspera(this.dataset.id)" data-id="' + item._id + '">Lista de Espera</button>');
