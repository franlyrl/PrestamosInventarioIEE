...
const userData = localStorage.getItem('utn_user');
const currentUser = JSON.parse(userData);
const nombreUsuario = currentUser?.nombre_completo || currentUser?.nombre || 'Usuario';
const userId = currentUser?._id || currentUser?.id || 'user001';
const rolUsuario = currentUser?.rol || currentUser?.rol_nombre || 'estudiante';
this.solicitudes = [
{
_id: 'ejemplo001',
usuario: {
...
{
id_insumo: {
_id: '69ae4a811c2b7671525a1f25',
NombProducto: 'Resistor SMD 0805 4.7k'
},
cantidad: 1,
caracteristicas: '±1%, 125mW'
}
],
estado: 'pendiente',
fecha_prestamo: '2026-04-08T14:58:50.193Z',
createdAt: '2026-04-08T14:58:50.193Z'
},
{
_id: 'ejemplo002',
usuario: {
...
{
id_insumo: {
_id: '69bf4a811c2b7671525a1f26',
NombProducto: 'Capacitor Cerámico 100nF 50V'
},
cantidad: 2,
caracteristicas: 'X7R, 5mm'
}
],
estado: 'aprobada',
fecha_prestamo: '2026-03-17T22:01:58.509Z',
createdAt: '2026-03-17T22:01:58.509Z'
}
];
console.log('Datos de ejemplo cargados para usuario:', nombreUsuario);
this.renderSolicitudes();
this.updateStatistics();
}
...
this.showToast('👁️ Abriendo detalles de la solicitud...', 'info');
// Implementar lógica para ver detalles
}

editarSolicitud(solicitudId) {
console.log('✏️ Editar solicitud:', solicitudId);
this.showToast('✏️ Abriendo editor de solicitud...', 'info');
...
this.showToast('❌ Cancelando solicitud...', 'error');
...
verSolicitud(solicitudId) {
console.log('Ver solicitud:', solicitudId);
...
return `
<div class="flex items-center gap-2 p-2 bg-slate-50 rounded">
<span class="text-lg">${elementos}</span>
</div>
`;
}
} else {
return '<p class="text-slate-500">No hay elementos</p>';
}
})()}
</div>
</div>
...
.catch(error => {
console.error('Error cancelando en API:', error);
this.showToast('Error al cancelar en la base de datos', 'error');
});
}

toggleMenu(solicitudId) {
console.log('Toggle menu:', solicitudId);
...
window.DesktopUserController = DesktopUserController;
window.desktopUserController = null;
...
// Verificar si solicitudesController está disponible
const controllerDisponible = window.solicitudesController && window.solicitudesController.solicitudes;
console.log('Soli_DeskUs.js: solicitudesController disponible:', controllerDisponible);
if (controllerDisponible) {
console.log('Soli_DeskUs.js: Datos disponibles en solicitudesController:', window.solicitudesController.solicitudes.length);
}
...
// Siempre cargar datos del API (no depender de solicitudesController)
console.log('Soli_DeskUs.js: Cargando datos desde API...');
